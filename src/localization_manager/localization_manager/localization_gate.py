...
class LocalizationGate(Node):
    def __init__(self):
        ...
        self.create_subscription(PoseWithCovarianceStamped, pose_topic, self.on_pose, 20)
    ...
    def evaluate(self):
        if self.last_pose is None:
            ...
            return
        pose_time = rclpy.time.Time.from_msg(self.last_pose.header.stamp)
        age = (self.get_clock().now() - pose_time).nanoseconds * 1e-9
        ...
        if pose_ok and age <= self.max_pose_age_sec:
            ...
            self.publish_status(True, "ready", "pose_ok")
            return
        if (
            pose_ok
            and self.latch_valid_pose
            and self.last_valid_pose_time is not None
            and (self.get_clock().now() - self.last_valid_pose_time).nanoseconds * 1e-9
            <= self.latched_pose_timeout_sec
        ):
            self.status_pub.publish(Bool(data=True))
            self.publish_status(True, "ready", "pose_latched")
            return
        self.status_pub.publish(Bool(data=False))
        if age > self.max_pose_age_sec:
            self.publish_status(False, "stale_pose", "pose_timeout")
            return
        self.publish_status(
            False,
            "covariance_rejected",
